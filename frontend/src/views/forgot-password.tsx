import { useState } from 'react';
import { Link } from 'react-router-dom';
import { Mail, AlertCircle, CheckCircle, ArrowLeft } from 'lucide-react';
import { API_URL } from '../config/api';
import '../css/global.css';
import '../css/forgot-password.css';

function ForgotPassword() {
  const [email, setEmail] = useState('');
  const [error, setError] = useState('');
  const [successMessage, setSuccessMessage] = useState('');
  const [loading, setLoading] = useState(false);
  const [isGoogleAccount, setIsGoogleAccount] = useState(false);

  const handleSubmit = async (e: React.SyntheticEvent) => {
    e.preventDefault();
    setError('');
    setSuccessMessage('');
    setIsGoogleAccount(false);
    setLoading(true);

    try {
      const res = await fetch(`${API_URL}/auth/forgot-password`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email }),
      });

      const data = await res.json();

      if (!res.ok) {
        setError(data.error || 'Something went wrong. Please try again.');
        return;
      }

      // Google-only account detected
      if (data.googleAccount) {
        setIsGoogleAccount(true);
        return;
      }

      setSuccessMessage(data.message || "If that email is registered, you'll receive a reset link shortly.");
    } catch (err) {
      console.error('Forgot password error:', (err as Error).message);
      setError('Something went wrong. Please try again.');
    } finally {
      setLoading(false);
    }
  };

  const handleGoogleLogin = () => {
    window.location.href = `${API_URL}/auth/google`;
  };

  return (
    <>
      <div className="forgot-password-body">
        <Link to="/login" className="back-to-home"><ArrowLeft size={16} strokeWidth={1.5} />Back to Login</Link>
        <div className="forgot-password-window">
          <h1>Forgot Password</h1>

          {isGoogleAccount ? (
            <div className="google-account-notice">
              <p className="google-account-title">This account uses Google Sign-In.</p>
              <p className="google-account-subtitle">
                Please use Continue with Google to log in instead.
              </p>
              <button className="google-btn" onClick={handleGoogleLogin} type="button">
                <svg className="google-icon" viewBox="0 0 24 24" width="18" height="18">
                  <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/>
                  <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                  <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
                  <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
                </svg>
                Continue with Google
              </button>
              <p className="forgot-password-link">
                <Link to="/login">Back to Login</Link>
              </p>
            </div>
          ) : (
            <>
              <p className="forgot-password-subtitle">
                Enter your email address and we'll send you a link to reset your password.
              </p>

              {successMessage && (
                <div className="forgot-password-success">
                  <CheckCircle size={18} strokeWidth={1.5} style={{ flexShrink: 0 }} />
                  {successMessage}
                </div>
              )}

              {error && (
                <div className="forgot-password-error">
                  <AlertCircle size={16} strokeWidth={1.5} />
                  {error}
                </div>
              )}

              <form onSubmit={handleSubmit}>
                <div className="input-group">
                  <label htmlFor="email">Email</label>
                  <div className="input-wrapper">
                    <Mail size={20} strokeWidth={1.5} className="input-icon" />
                    <input
                      type="email"
                      id="email"
                      value={email}
                      placeholder="Enter your email address"
                      onChange={(e) => setEmail(e.target.value)}
                      required
                    />
                  </div>
                </div>
                <button className="forgot-password-btn" type="submit" disabled={loading}>
                  {loading ? 'Sending...' : 'Send Reset Link'}
                </button>
              </form>

              <p className="forgot-password-link">
                Remember your password? <Link to="/login">Log in</Link>
              </p>
            </>
          )}
        </div>
      </div>
    </>
  );
}

export default ForgotPassword;
